...
pub async fn send_http_request<R: DeserializeOwned>(
    ...
    let client = request_params
        .proxy
        .as_ref()
        .map_or_else(Client::new, |proxy| {
            Client::builder()
                .proxy(proxy.clone())
                .build()
                .unwrap_or_else(|err| {
                    log::error!("Failed to build a client with proxy: {proxy:?}. Error: {err}");
                    Client::new()
                })
        });
    ...
    let response = request
        .send()
        .await
        .inspect_err(|e| log::error!("Request failed: {}", e))?;

    let text = response
        .text()
        .await
        .inspect_err(|e| log::error!("Failed to retrieve response text: {}", e))?;

    let deserialized_body = serde_json::from_str::<R>(&text)
        .inspect_err(|e| log::error!("Failed to deserialize response: {}\n {} ", e, text))?;
